...
- [ ] 14. При входе в комнату сначала запросить `GET /api/boards/[id]/records`.
- [ ] 15. «Холодный» старт: если `liveRecords.size === 0` — заполнить tldraw-store и Liveblocks Storage данными из БД. Иначе — взять живое состояние Liveblocks.
- [ ] 16. Применить `tldraw_schema` из БД (миграции схемы tldraw при необходимости).
- [ ] 17. Снять зависимость загрузки от старого слепка Liveblocks: источник правды — БД, Liveblocks — транспорт.
- [ ] 18. Тесты: (а) первый вход после рестарта — данные из БД; (б) второй пользователь видит то же; (в) правки сохраняются обратно в БД.
